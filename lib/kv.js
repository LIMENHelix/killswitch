...
export function keyFor(key, tenantId) {
  ...
  let scoped = key;
  if (t && !key.startsWith('ks:t:')) scoped = 'ks:t:' + t + ':' + key.slice(3);
  // A preview can exercise every data path without seeing or mutating the live
  // customer keyspace, even when Vercel injects the same Redis credentials.
  if (process.env.VERCEL_ENV === 'preview' && !scoped.startsWith('ks:env:preview:')) {
    scoped = 'ks:env:preview:' + scoped.slice(3);
  }
  return scoped;
}
