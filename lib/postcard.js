...
//
// This used to be `process.env.KS_PHONE || ''`, and KS_PHONE is EMPTY in
// production, so every postcard mailed so far went out with no way to phone us
// on it. A piece of physical mail that asks a shop owner to act, and then gives
// them nothing to call, is a stamp spent on nothing. The real numbers are the
// default now rather than an env var somebody has to remember to set.
//
// KS_PHONE still overrides, and now takes a comma-separated list, so the numbers
// can be changed without a deploy. Nothing was taken away.
const PHONES = (process.env.KS_PHONE || '913-948-3747, 913-933-1687')
