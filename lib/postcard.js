...
export function frontHtml(lead = {}) {
  ...
  return `<html><head><meta charset="utf-8"><style>
    ...
    ${PHONE ? `<div class="ph"><div class="l">Call us</div><div class="n">${PHONE}</div></div>` : ''}
  </div></body></html>`;
}
...
export function backHtml(lead) {
  ...
  const phoneLine = PHONE
    ? `<div class="cta2">Or call ${PHONE}${url ? ' and we will walk you through it.' : ' and say "free site."'}</div>`
    : '';
