...
import { getLeads, saveLeads, getConfig, saveConfig } from './store.js';
...
export async function lobSend(lead) {
  ...
  const form = new URLSearchParams({
    ...
    front: frontHtml(), back: backHtml(lead), size: '6x9',
  });
...
export async function runAutopilot(reason = 'cron') {
  ...
  const spent = mailedCount * COST;
  const budgetLeft = (cfg.budgetCeiling || 0) - spent;

  if (budgetLeft <= 0) {
    cfg.enabled = false; // trip the killswitch so it can't overrun
    cfg.lastRun = { when: stamp, mailed: 0, note: 'budget ceiling reached — autopilot switched OFF', reason };
    await saveConfig(cfg);
    return { ran: false, reason: 'budget ceiling reached', disabled: true };
  }
