...
export async function lobSend(lead, suppressionState) {
...
export async function runAutopilot(reason = 'cron') {
  ...
  let sent = 0, bad = 0, suppressed = 0, attempts = 0; const failed = [];
  ...
  const [meta, suppressionState] = target > 0
    ? await Promise.all([getLeadMeta(), getSuppressionState()])
    : [{}, {}];
  if (target > 0) {
  ...
  cfg.lastRun = {
    when: stamp, reason, mailed: sent, bad, suppressed,
