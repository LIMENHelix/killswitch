...
export async function stripeDelete(path, params) {
  if (!externalSideEffectsAllowed()) return { error: 'preview_side_effects_disabled' };
  const k = key();
...
export async function stripePost(path, params, options = {}) {
  if (!externalSideEffectsAllowed()) return { error: 'preview_side_effects_disabled' };
  const k = key();
  if (!k) return { error: 'not_configured' };
  ...
  const headers = { Authorization: 'Bearer ' + k, 'content-type': 'application/x-www-form-urlencoded' };
