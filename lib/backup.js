...
export async function checkUptime(base, sites, fetchFn = fetch, now = new Date()) {
  ...
  // Bounded parallel batches keep one slow site from blocking every customer,
  // while avoiding an unbounded burst when the catalogue grows.
  const BATCH = 10;
  for (let i = 0; i < sites.length; i += BATCH) {
    const rows = await Promise.all(sites.slice(i, i + BATCH).map(async (s) => {
      const url = base + '/s/' + s.slug;
      let ok = false, status = 0;
      try {
        const r = await fetchFn(url, { method: 'GET', redirect: 'manual', signal: AbortSignal.timeout(8000) });
        status = r.status;
        ok = r.status >= 200 && r.status < 400;
      } catch { status = 0; }
      return { s, ok, status };
    }));
    for (const { s, ok, status } of rows) {
