...
import { siteForEmail } from './sites.js';
import { recordLifecycle, reconcileLifecycle } from './lifecycle.js';
...
export async function onboardCustomer({ email, site, name, phone, source, leadId, dealCents }) {
  ...
  const lifecycleSite = link.linked ? await siteForEmail(e) : null;
  const lifecycleSource = String(source || 'onboard').slice(0, 80);
  if (lifecycleSite && lifecycleSite.published) {
    await recordLifecycle(e, {
      type: 'site.published', stage: 'site_published',
      idempotencyKey: lifecycleSource + ':site-published:' + lifecycleSite.slug,
      data: { siteSlug: lifecycleSite.slug, source: lifecycleSource },
    });
  }
  await recordLifecycle(e, {
    type: 'customer.onboarded', stage: 'onboarded',
    idempotencyKey: lifecycleSource + ':onboarded',
    data: { source: lifecycleSource, siteSlug: link.slug || '', linked: link.linked, live: link.live },
  });
  await reconcileLifecycle({
    email: e, account, site: lifecycleSite || {},
    idempotencyKey: lifecycleSource + ':post-onboard',
  });
